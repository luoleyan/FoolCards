...
describe('游戏黑盒测试', () => {
  ...
  test('玩家出牌测试', () => {
    ...
    // 检查出牌后的状态
    expect(playerHand.children.length).toBe(initialHandCount - 1);
  ...
  test('玩家换牌测试', () => {
    ...
    // 检查换牌后的状态
    expect(playerHand.children.length).toBe(initialHandCount);
  ...
  test('回合结束测试', () => {
    ...
    expect(playerHand.children.length).toBe(5); // 应该重新发牌
    expect(opponentHand.children.length).toBe(5); // 应该重新发牌
