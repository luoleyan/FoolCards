  ...
  analyzePerformance(testResults) {
    ...
    for (const suite of testSuites) {
      ...
      for (const test of suite.testResults) {
        ...
        // 收集慢测试和快测试
        if (duration > 0.5) { // 超过500ms的测试视为慢测试
          slowTests.push(testData);
        } else if (duration < 0.01) { // 小于10ms的测试视为快测试
    ...
    // 计算标准差
    const squaredDiffs = performanceData.map(test => Math.pow(test.duration - averageDuration, 2));
  ...
  _generateRecommendations(slowTests, suitePerformance, averageDuration, medianDuration) {
    ...
    if (slowTests.length > 0) {
      recommendations.push({
        ...
        impact: '高',
        targets: slowTests.slice(0, 5).map(test => `${test.suiteName}: ${test.testName} (${test.duration.toFixed(2)}s)`)
      });
    }
    ...
    if (slowSuites.length > 0) {
      recommendations.push({
        ...
        impact: '中',
        targets: slowSuites.slice(0, 3).map(suite => `${suite.suiteName} (${suite.totalDuration.toFixed(2)}s, ${suite.testCount}个测试)`)
      });
    }
