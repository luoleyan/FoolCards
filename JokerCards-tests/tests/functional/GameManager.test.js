...
describe('GameManager功能测试', () => {
  ...
  test('开始新回合', () => {
    expect(gameManager._currentRound).toBe(0);
    gameManager.startNewRound();
    expect(gameManager._currentRound).toBe(1);
    expect(gameManager._exchangeCount).toBe(5);
  });

  test('出牌功能', () => {
    const card = cards[0];
    const result = gameManager.playCard(card, 0);
    expect(result).toBe(true);
    expect(gameManager.playAreas[0].children).toContain(card.node);
  ...
  test('回合出牌限制', () => {
    // 出三张牌
    gameManager.playCard(cards[0], 0);
    gameManager.playCard(cards[1], 1);
    gameManager.playCard(cards[2], 2);
    ...
    // 使用额外出牌次数
    gameManager.playCard(cards[3], 0);
  ...
  test('分数计算', () => {
    ...
    gameManager.playCard(card1, 0);
    gameManager.playCard(card2, 0);
  ...
  test('换牌功能', () => {
    // 初始换牌次数
    const initialExchangeCount = gameManager.getExchangeCount();
    ...
    expect(gameManager.getExchangeCount()).toBe(initialExchangeCount - 1);
    ...
    // 检查换牌次数是否为0
    expect(gameManager.getExchangeCount()).toBe(0);
  ...
  test('游戏结束', () => {
    // 设置当前回合为最大回合数
    gameManager._currentRound = gameManager.maxRounds;
