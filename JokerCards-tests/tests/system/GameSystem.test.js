...
describe('游戏系统测试', () => {
  ...
  test('完整游戏流程测试', () => {
    ...
    // 检查玩家出牌后的状态
    expect(playerHand.children.length).toBeLessThan(5);
    ...
    // 检查玩家继续出牌后的状态
    expect(playerHand.children.length).toBeLessThan(4);
    ...
    expect(totalAICards).toBeGreaterThan(2); // 玩家出了2张牌，AI应该也出牌了
    ...
    if (playerHand.children.length > 0 && gameManager.exchangeCount > 0) {
      ...
      // 检查换牌后的状态
      expect(gameManager.exchangeCount).toBe(4);
  ...
  test('特殊牌型加分测试', () => {
    ...
    const expectedBonusScore = 20; // 同花加成
    
    // 玩家得分应该包含基础分和加成分
    expect(gameManager.playerScore).toBeGreaterThanOrEqual(expectedBaseScore + expectedBonusScore);
  });
  ...
  test('场景效果组合测试', () => {
    ...
    const expectedBonusScore = 15; // JQK加成
    const expectedFlushBonus = 20; // 同花加成（因为都是红桃）
  ...
  test('换牌机制测试', () => {
    ...
    if (playerHand.children.length > 0) {
      ...
      // 检查是否获得了新卡牌
      expect(playerHand.children.length).toBe(initialHandCards.length);
      
      // 检查手牌是否发生变化
      const currentHandCards = playerHand.children;
      let hasNewCard = false;
      
      for (let i = 0; i < currentHandCards.length; i++) {
        if (!initialHandCards.includes(currentHandCards[i])) {
          hasNewCard = true;
          break;
        }
      }
