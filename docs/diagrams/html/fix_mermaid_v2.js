      ...
      try {
        mermaid.run({
          ...
          document.querySelectorAll('.mermaid').forEach(el => {
            el.innerHTML = '<div class="error-message"><h3>图表渲染错误</h3><p>' + error.message + '</p><pre>' + el.textContent + '</pre></div>';
          });
        });
      } catch (error) {
        console.error('Mermaid处理错误:', error);
        document.querySelectorAll('.mermaid').forEach(el => {
          el.innerHTML = '<div class="error-message"><h3>图表处理错误</h3><p>' + error.message + '</p><pre>' + el.textContent + '</pre></div>';
        });
      }
    });
  </script>`;
...
function fixMermaidSyntax(mermaidCode) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('classDef')) {
      ...
      // 确保每个classDef都在单独的行上
      if (line.match(/classDef.*classDef/)) {
        const parts = line.split(/(?=classDef)/g);
        for (const part of parts) {
          if (part.trim()) {
            fixedLines.push('    ' + part.trim().replace(/;$/, ''));
          }
        }
        continue; // 跳过当前行，因为已经处理了所有部分
      }
    }
    ...
    if (line.match(/^\s*class\s+/)) {
      ...
      if (line.match(/class.*class/)) {
        const parts = line.split(/(?=class\s+)/g);
        for (const part of parts) {
          if (part.trim()) {
            fixedLines.push('    ' + part.trim().replace(/;$/, ''));
          }
    ...
    // 修复flowchart中的箭头语法
    if (chartType === 'flowchart' && line.includes('-->')) {
      // 确保箭头两侧有空格
      line = line.replace(/(\w)-->(\w)/g, '$1 --> $2');
    }
    
    // 修复ER图中的关系语法
    if (chartType === 'er' && line.includes('||--')) {
      // 确保关系两侧有空格
      line = line.replace(/(\w)\|\|--/g, '$1 ||--');
      line = line.replace(/--\|\|(\w)/g, '--|| $1');
      line = line.replace(/--o\{(\w)/g, '--o{ $1');
    }
